...
Write-Verbose -Verbose "==================================="
	...
	$event.WriteEntry("DirServer Script Executed", $info_event, 5001)
...
Invoke-Command  -Credential $credential -ComputerName $env:COMPUTERNAME -ScriptBlock {
	...
	Write-Verbose -Verbose "Entering DirServer Script"
	...
	$event.Source = "AzureEnvironment"
	$info_event = [System.Diagnostics.EventLogEntryType]::Information
	$event.WriteEntry("In DirServer scriptblock", $info_event, 5001)

	#go to our packages scripts folder
	Set-Location $workingDir
	
	$zipfile = $workingDir + "\SDP.zip"
	$destination = $workingDir
	[System.Reflection.Assembly]::LoadWithPartialName("System.IO.Compression.FileSystem") | Out-Null
	[System.IO.Compression.ZipFile]::ExtractToDirectory($zipfile, $destination)
	
	Write-Verbose -Verbose "Installing SDP Software"
    Start-Process -FilePath C:\Windows\System32\msiexec.exe -ArgumentList "/i StartReadyDeploymentConsole.Install.msi /qb" -Wait
    Start-Process -FilePath C:\Windows\System32\msiexec.exe -ArgumentList "/i StartReadyDirectorConsole.Install.msi /qb" -Wait
    Start-Process -FilePath C:\Windows\System32\msiexec.exe -ArgumentList "/i StartReadyDownloadConsole.Install.msi /qb" -Wait
    Start-Process -FilePath C:\Windows\System32\msiexec.exe -ArgumentList "/i StartReadyInitConsole.Install.msi /qb" -Wait
